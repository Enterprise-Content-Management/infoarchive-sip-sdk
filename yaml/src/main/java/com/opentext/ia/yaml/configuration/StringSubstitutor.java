...
public class StringSubstitutor implements Visitor {
  ...
  public void accept(Visit visit) {
    boolean isResourceContainer = InfoarchiveYamlConstant.RESOURCE_CONTAINER_PATHS.stream()
        .anyMatch(visit.getPath()::matches);
    YamlMap yaml = visit.getMap();
    yaml.entries()
        .filter(entry -> entry.getValue().isString())
        .filter(entry -> !isResourceContainer || !InfoarchiveYamlConstant.TEXT.equals(entry.getKey()))
