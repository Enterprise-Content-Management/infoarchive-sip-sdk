...
import com.emc.ia.sdk.support.io.IOStreams;
...
public class ContentAssemblerDefault<D> implements ContentAssembler<D> {
  ...
  protected ContentInfo addContent(DigitalObject digitalObject) throws IOException {
    try (InputStream stream = digitalObject.get()) {
      Collection<EncodedHash> hashes = zip.addEntry(digitalObject.getReferenceInformation(), stream, contentHashAssembler);
