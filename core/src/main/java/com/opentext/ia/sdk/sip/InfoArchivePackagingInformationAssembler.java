...
/**
 * Convert {@linkplain PackagingInformation} into a format accepted by
 * InfoArchive.
 */
public class InfoArchivePackagingInformationAssembler extends PrintWriterAssembler<PackagingInformation> {
	...
	protected void add(PackagingInformation packagingInformation, PrintWriter writer) {
		XmlBuilder<?> builder = XmlBuilder.newDocument(writer).namespace("urn:x-emc:ia:schema:sip:1.0").element("sip");
		...
		builder.element("dss").element("holding", dss.getHolding()).element("id", dss.getId())
				.element("pdi_schema", dss.getSchema()).element("production_date", Dates.toIso(dss.getProductionDate()))
				.element("base_retention_date", Dates.toIso(dss.getBaseRetentionDate()))
				.element("producer", dss.getProducer()).element("entity", dss.getEntity())
				.element("priority", Integer.toString(dss.getPriority())).element("application", dss.getApplication())
				.element("retention_class", dss.getRetentionClass()).end()
		...
		packagingInformation.pdiHash()
				.ifPresent(hash -> builder.element("pdi_hash").attribute("algorithm", hash.getHashFunction())
						.attribute("encoding", hash.getEncoding()).text(hash.getValue()).end());
		if (!packagingInformation.getCustomAttributes().isEmpty()) {
			builder.element("custom").element("attributes");
			packagingInformation.getCustomAttributes()
					.forEach((name, value) -> builder.element("attribute").attribute("name", name).text(value).end());
			builder.end().end();
		}
		builder.build();
	}
