...
- `YamlMap.entries()` are again sorted by key, but now in the same order that the `sort()` method would sort them.



== 7.2.0 - 2017-09-01

*NOTE*: `YamlMap.entries()` are no longer sorted by key. That wasn't part of the contract, so it's not technically
backwards incompatible, but it might be something that client code depended on.



=== Added

- Addded `YamlMap.sort()` overload that allow keeping the top level unsorted
