...
- During YAML normalization, binary resources are not inlined. Use `binary: true` to indicate a binary resource, or
use a `format` or file extension that is considered binary. For now, only `zip` is supported.
